...
/*
========================================================
ADMIN - IMPORT CASES (2 SHEET FORMAT)
========================================================
*/
// exports.importCaseStudiesFromExcel = async (req, res) => {
...
//     const caseSheet = workbook.Sheets["CaseInfo"]
//     const questionSheet = workbook.Sheets["Questions"]

//     if (!caseSheet || !questionSheet) {
//       return res.status(400).json({
//         message: "Both CaseInfo and Questions sheets are required"
//       })
//     }
...
//     if (!caseData.length) {
//       return res.status(400).json({ message: "CaseInfo sheet is empty" })
//     }

//     if (!questionData.length) {
//       return res.status(400).json({ message: "Questions sheet is empty" })
//     }

//     const createdCases = []

//     // Loop through each case
//     for (const caseRow of caseData) {
...
//       if (!caseSlug || !caseTitle) {
//         throw new Error("caseSlug and caseTitle are required")
//       }

//       // Check duplicate
//       const existingCase = await CaseStudy.findOne({ slug: caseSlug })
//       if (existingCase) {
//         continue
//       }

//       // Filter questions for this case
//       const relatedQuestions = questionData.filter(
//         q => q.caseSlug === caseSlug
//       )

//       if (!relatedQuestions.length) {
//         throw new Error(`No questions found for caseSlug: ${caseSlug}`)
//       }
...
//       // Prepare Question Docs
//       const questionDocs = relatedQuestions.map(q => ({
//         caseStudyId: newCase[0]._id,
...
exports.importCaseStudiesFromExcel = async (req, res) => {
  try {
    ...
    //   let caseDoc = await CaseStudy.findOne({ slug: caseSlug })

    //     if (!caseDoc) {
    //       caseDoc = await CaseStudy.create({
    //         title: caseTitle,
    //         slug: caseSlug,
    //         openingImageUrl,
    //         revealImageUrl,
  ...
  const { caseSlug, caseTitle, openingImageUrl, revealImageUrl, maxAttempts, isActive } = caseRow
